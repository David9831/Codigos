...
def evaluate_actor(actor, env, num_episodes=1, fixed_action=None):
    """
    Evalúa el actor en el entorno y grafica la tensión en el nodo 6 y las acciones tomadas.

    Args:
        actor: El modelo del actor entrenado (puede ser None si se usa acción fija).
        env: El entorno IEEE33BusSystem.
        num_episodes: Número de episodios para evaluar.
        fixed_action: Valor de la acción fija a usar (si no es None).
    """
...
if __name__ == "__main__":
    ...
    try:
        ...
        #exit() #Comentamos el exit para que pueda ejecutar la accion fija
        loaded_actor = None # Si no se encuentra el modelo, lo dejamos en None
    ...
    # Definir la acción fija
    fixed_action = 3  # Cambia este valor a la acción que desees

    # Evaluar el actor (o usar la acción fija)
    all_episode_voltages, all_episode_actions = evaluate_actor(loaded_actor, env, num_episodes=1, fixed_action=fixed_action)
